...
parser.add_argument("--N_train", type=str, required=True,
                    help="identifier for dataset size, e.g. '10k'")
...
#--weight_decay 0.001 --layer_size 3 60 2 --activation 'relu'  --transform 'relu' --result_root results   --model_no 0 --N_train 10k --data_file data_10k.pkl
#--layer_sizes 3 60 2 --activation relu --transform relu --data_file data_10k_rho_0.6.pkl --N_train 10k_rho_0.6 --weight_decay 0.001 --model_no 0 --result_root results
    print(args)
    if args.separate:
        model = models.TasteNetChoiceSep(args)
    else:
        model = models.TasteNetChoice(args)
    ...
    loss_train_list, loss_dev_list, best_model, _ = train(model, data_train, data_dev, args)
