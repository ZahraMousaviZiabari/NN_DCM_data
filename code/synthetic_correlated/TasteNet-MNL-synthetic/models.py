...
class TasteNetChoiceSep(nn.Module):
    """
    Two separate TasteNets for time and wait.
    This is referenced in your main script but missing.
    """
    ...
    def forward(self, z, x):
        b_time = self.taste_time(z)     # (N, 1)
        b_wait = self.taste_wait(z)     # (N, 1)
        b = torch.cat([b_time, b_wait], dim=1)  # (N, 2)
        return self.util(x, b)
