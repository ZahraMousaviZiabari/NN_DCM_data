...
# network structure
parser.add_argument("--taste_params", type=int,
                    default=8)  # number of taste parameters, output size of the last hidden layer , fixed
parser.add_argument("--K", type=int,
                    default=4)  # input size of choice model (alternative attributes, including intercept 1)
...
if (args.kfold == False):
    ...
    args.seed = 42
    args.hidden_sizes = [110]
    args.result_root = "results/CV"
...
# Model result path
args.result_path = args.result_root + "/model" + str_hidden_sizes + str_act + str_transform + str_l1 + str_l2
...
if (args.kfold == False):
    ...
    pickle.dump(s_all, open(os.path.join(args.result_path, "summary_all_100runs_new.pkl"), "wb"))
    pickle.dump(b_all, open(os.path.join(args.result_path, "b_all_100runs_new.pkl"), "wb"))
    pickle.dump(models_all, open(os.path.join(args.result_path, "models_all_100runs_new.pkl"), "wb"))
else:
    #Usage: --l1 0.0 --l2 0.0 --ac_func relu
    fold_acc = []
    fold_f1 = []
    fold_nlls_train = []
    fold_nlls_test = []
    ...
    models_all = []
    args.data_path = args.data_path + "/CV"
    for no in range(5):
        # ======Prepare input data ========
        args.data_train = "train_split"+str(no)+".pkl"
        args.data_test = "test_split"+str(no)+".pkl"
        data_train, data_test = csv_to_dict(dir = args.data_path, filename="split"+str(no), save = True)
        ds_train = ChoiceDataset(args.data_path, args.data_train)
        ds_test = ChoiceDataset(args.data_path, args.data_test)
        ...
        nll_train = summary['train']['nll']
        nll_test = summary['test']['nll']
        accuracy = summary['test']['acc']*100
        f1 = summary['test']['f1']*100
        ...
        b_test = predictParams(best_model, ds_test.z)
    ...
    print(
        f"CV Acc (test): "
        f"mean={np.mean(fold_acc):.5f}, "
        f"std={np.std(fold_acc):.5f}"
    )

    print(
    f"CV f1_score (test): "
    f"mean={np.mean(fold_f1):.5f}, "
    f"std={np.std(fold_f1):.5f}"
    )
